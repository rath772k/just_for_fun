...
class CausalSelfAttention(nn.Module):
    ...
    **NOTE:** In the original Attention is All You Need paper, values is of a different dimension which is then
    ...
    def __init__(self, embed_size, attn_size, context_length):
        ...
        self.register_buffer("tril", torch.tril(torch.ones(context_length, context_length)).view(1, context_length, context_length))
    ...
    def forward(self, embeddings):
        ...
        scores = q @ k.transpose(-2, -1) * (1 / math.sqrt(float(attn_size))) # (batch_size, context_length, context_length)
        masked_scores = torch.where(self.tril[:, :context_length, :context_length] == 0, float("-inf"), scores) # (batch_size, context_length, context_length)
...
class Transformer(nn.Module):
    def __init__(self, vocab_size, embed_size, num_heads, context_length, num_layers):
        ...
        self.pos_embedding = nn.Embedding(num_embeddings=context_length, embedding_dim=embed_size)
        self.attn_blocks = nn.ModuleList((AttentionBlock(embed_size, num_heads, context_length) for _ in range(num_layers)))
    ...
    def forward(self, inputs, targets=None):
        ...
        x = self.token_embedding(inputs) + self.pos_embedding(torch.arange(context_length, device=inputs.device))
    ...
    def generate(self, start_idx, max_tokens=100):
        for _ in range(max_tokens):
            logits, _ = self(start_idx[:, -self.context_length:]) # we only care till context length
